...
for (const scene of [new basic_scene_1.BasicScene()]) {
    describe(`(${scene}): correctly infers submit info from commits`, function () {
        ...
        it('can infer title/body from single commit', async () => {
            ...
            (0, chai_1.expect)(await (0, pr_title_1.getPRTitle)({ branchName: 'a', editPRFieldsInline: false }, scene.getContext())).to.equals(title);
            (0, chai_1.expect)((0, pr_body_1.inferPRBody)({ branchName: 'a', template: 'template' }, scene.getContext()).inferredBody).to.equals(`template`);
            scene
                .getContext()
                .userConfig.update((data) => (data.submitIncludeCommitMessages = true));
            (0, chai_1.expect)(await (0, pr_title_1.getPRTitle)({ branchName: 'a', editPRFieldsInline: false }, scene.getContext())).to.equals(title);
            (0, chai_1.expect)((0, pr_body_1.inferPRBody)({ branchName: 'a', template: 'template' }, scene.getContext()).inferredBody).to.equals(`${body}\n\ntemplate`);
        });
        it('can infer just title with no body', async () => {
            ...
            (0, chai_1.expect)(await (0, pr_title_1.getPRTitle)({ branchName: 'a', editPRFieldsInline: false }, scene.getContext())).to.equals(title);
            (0, chai_1.expect)((0, pr_body_1.inferPRBody)({ branchName: 'a', template: 'template' }, scene.getContext()).inferredBody).to.equal('template');
        });
        it('can infer title/body from multiple commits', async () => {
            ...
            (0, chai_1.expect)(await (0, pr_title_1.getPRTitle)({ branchName: 'a', editPRFieldsInline: false }, scene.getContext())).to.equals(title);
            (0, chai_1.expect)((0, pr_body_1.inferPRBody)({ branchName: 'a' }, scene.getContext()).inferredBody).to.equal(``);
            scene
                .getContext()
                .userConfig.update((data) => (data.submitIncludeCommitMessages = true));
            (0, chai_1.expect)(await (0, pr_title_1.getPRTitle)({ branchName: 'a', editPRFieldsInline: false }, scene.getContext())).to.equals(title);
            (0, chai_1.expect)((0, pr_body_1.inferPRBody)({ branchName: 'a' }, scene.getContext()).inferredBody).to.equal(`${title}\n\n${secondSubj}`);
        });
