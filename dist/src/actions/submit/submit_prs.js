...
async function submitPullRequest(args, context) {
    const pr = (await requestServerToSubmitPRs(args.cliAuthToken, args.submissionInfo, context))[0];
    if (pr.response.status === 'error') {
        throw new errors_1.ExitFailedError(`Failed to submit PR for ${pr.response.head}: ${parseSubmitError(pr.response.error)}`);
    }
...
async function requestServerToSubmitPRs(cliAuthToken, submissionInfo, context) {
    const response = await retyped_routes_1.request.requestWithArgs(server_1.API_SERVER, graphite_cli_routes_1.API_ROUTES.submitPullRequests, {
