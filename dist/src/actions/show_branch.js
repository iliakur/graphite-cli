...
async function showBranchInfo(branchName, opts, context) {
    ...
    output.push('');
    output.push((0, show_commits_1.showCommits)(context.metaCache.isTrunk(branchName)
        ? `${branchName}~`
        : context.metaCache.getBaseRevision(branchName), branchName, opts.patch && !opts.diff));
    if (opts.diff) {
        output.push('');
        output.push((0, diff_1.showDiff)(context.metaCache.isTrunk(branchName)
            ? `${branchName}~`
            : context.metaCache.getBaseRevision(branchName), branchName));
    }
