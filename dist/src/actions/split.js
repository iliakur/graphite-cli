...
async function splitCurrentBranch(args, context) {
    ...
    const children = context.metaCache.getRelativeStack(branchToSplit, scope_spec_1.SCOPE.UPSTACK_EXCLUSIVE);
    context.metaCache.applySplitToCommits({
        branchToSplit,
        ...split,
    });
    (0, restack_1.restackBranches)(children, context);
}
...
async function splitByHunk(branchToSplit, context) {
    ...
    try {
        ...
        const defaultCommitMessage = context.metaCache
            ...
            .reverse()
            .join('\n\n');
        for (let unstagedChanges = context.metaCache.getUnstagedChanges(); unstagedChanges.length > 0; unstagedChanges = context.metaCache.getUnstagedChanges()) {
            context.splog.info(instructions);
            context.splog.newline();
