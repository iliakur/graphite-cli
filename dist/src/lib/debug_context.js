...
function captureState(context) {
    ...
    const state = {
        ...
        currentBranchName: (0, branch_ops_1.getCurrentBranchName)(),
    };
...
function recreateState(stateJson, splog) {
    ...
    fs_extra_1.default.writeFileSync(path_1.default.join(process.cwd(), 'first.txt'), 'first');
    (0, runner_1.runGitCommand)({
        args: ['add', 'first.txt'],
        onError: 'throw',
    ...
    if (state.currentBranchName) {
        (0, branch_ops_1.switchBranch)(state.currentBranchName);
        (0, branch_ops_1.deleteBranch)(tmpTrunk);
    }
    else {
        splog.warn(`No currentBranchName found, retaining temporary trunk.`);
        (0, branch_ops_1.switchBranch)(tmpTrunk);
    }
...
function recreateCommits(opts, splog) {
    ...
    const treeSha = (0, runner_1.runGitCommand)({
    ...
    while (commitsToCreate.length > 0) {
        ...
        const newCommitRef = (0, runner_1.runGitCommand)({
            args: [
                'commit-tree',
                treeSha,
                '-m',
